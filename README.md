...
* **Test-Ready:** Every required element has the exact `data-testid` 
* **Semantic HTML:** Built using proper tags (`<article>`, `<time>`, real `<button>` elements) instead of relying only on divs.
* **Accessible:** Fully keyboard navigable. You can tab through the checkbox and buttons easily.
* **Responsive:** Stacks nicely to fit mobile screens and keeps a clean layout on desktops.
* **Vanilla JS:** Uses raw JavaScript to handle the time remaining calculation and the checkbox toggle state.
